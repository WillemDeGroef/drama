...
public class EditFrame extends JFrame {
	...
	private void init(final ActionManager _actionManager) {
		_actionManager.setIdeBlackWhiteAction(new ActionImpl() {
			public void react() { setDecorator((byte) 0); }
		});
		_actionManager.setIdeLightAction(new ActionImpl() {
			public void react() { setDecorator((byte) 1); }
		});
		_actionManager.setIdeDarkAction(new ActionImpl() {
			public void react() { setDecorator((byte) 2); }
		});
	...
	private JPanel createTextArea() {
		...
		item.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				((AbstractAction) _textPane.getActionMap().get("Undo")).actionPerformed(null);
			}
		});
		popup.add(item);
		...
		item.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				((AbstractAction) _textPane.getActionMap().get("Redo")).actionPerformed(null);
			}
		});
		popup.add(item);
		...
		_textPane.getDocument().addDocumentListener(new DocumentListener() {
			...
			public void insertUpdate(DocumentEvent arg0) { this.notifyUpdate(); }
			public void removeUpdate(DocumentEvent arg0) { this.notifyUpdate(); }
		...
		_textPane.getDocument().addUndoableEditListener(new UndoableEditListener() {
			public void undoableEditHappened(UndoableEditEvent arg0) {
				if (arg0.getEdit().getPresentationName() == "addition" || arg0.getEdit().getPresentationName() == "deletion")
	...
	private JScrollPane createStatusWindow() {
		...
		JScrollPane scrollPane = new JScrollPane(_statusWindow,
				JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
				JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	...
	public void reset() {
		((AbstractAction) _textPane.getActionMap().get("Reset")).actionPerformed(null);
	}
}
